...
{
	int reuse = 1;
    //const size_t sklen = 128;
	/* Server's key: */
	NEWZ(sp); /* secret key (a random exponent) */
	NEWZ(S); /*Server public key*/
	dhGen(sp,S);
	NEWZ(C_prime);
	...
	deserialize_mpz(C_prime, sockfd);
	dhFinal(sp,S,C_prime,kA,klen);

	//for (size_t i = 0; i < klen; i++) {
	//	printf("%02x ",kA[i]);
	//}

	mpz_get_str(NULL,10,kA);

	sleep(2);

	fprintf(stderr, "Sending server key to socket...\n");

	serialize_mpz(sockfd, S);
...
{
    /* Client's key: */
    //const size_t cklen = 128;
	NEWZ(cp); /* secret key (a random exponent) */
	NEWZ(C); /*Client public key*/
	NEWZ(S_prime);
	...
	serialize_mpz(sockfd, C);
	dhGen(cp,C);
    
	sleep(7); /*wait for server to send*/
	
	fprintf(stderr, "Getting server key from socket...\n");

	deserialize_mpz(S_prime, sockfd);
	dhFinal(cp,C,S_prime,kB,klen);

	mpz_get_str(NULL,10,kB);
...
int main(int argc, char *argv[])
{
	...
	if (isclient) {
		...
		memcpy(prvKeyC,kB,klen);
		printf("\nClient init complete\n~~~~~~~~~~~~~~~~~~~~~~~\n");
	}
	else {
		printf("\nServer init started\n~~~~~~~~~~~~~~~~~~~~~~~\n");
		initServerNet(port);
		memcpy(prvKeyS,kA,klen);
